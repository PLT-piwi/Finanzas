...
export default function AddTransactionView({ addType, setAddType, form, setForm, onAddTransaction }) {
  ...
  return (
    <section className="view active">
      <header className="view__header">
        <h1 className="view__title">Agregar transacción</h1>
      </header>
      <div className="card form-card">
        <div className="type-toggle">
          <button className={`type-btn ${addType === 'expense' ? 'active' : ''}`} onClick={() => setAddType('expense')}>
            Gasto
          </button>
          <button className={`type-btn ${addType === 'income' ? 'active' : ''}`} onClick={() => setAddType('income')}>
            Ingreso
          </button>
        </div>
